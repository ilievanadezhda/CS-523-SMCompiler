...
class Share:
    ...
    def __init__(self, value, *args, **kwargs):
        ...
        self.value = value % self.FIELD_MODULUS
    ...
    def __add__(self, other):
        return Share((self.value + other.value) % self.FIELD_MODULUS)
...
def share_secret(secret: int, num_shares: int) -> List[Share]:
    ...
    shares = [Share(randint(0, Share.FIELD_MODULUS)) for _ in range(num_shares - 1)]
    shares.append(Share((secret - sum([share.value for share in shares])) % Share.FIELD_MODULUS))
    return shares


def reconstruct_secret(shares: List[Share]) -> int:
    """Reconstruct the secret from shares."""
    return sum([share.value for share in shares]) % Share.FIELD_MODULUS
